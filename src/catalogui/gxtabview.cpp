...
bool wxGxTab::OnDropObjects(wxCoord x, wxCoord y, const wxArrayString& GxObjects, bool bIsControlOn, bool bIsShiftOn)
{
	IViewDropTarget* pViewDropTarget = dynamic_cast<IViewDropTarget*>(m_pCurrentWnd);
	if(!pViewDropTarget)
		return wxDragNone;
	return pViewDropTarget->OnDropObjects(x, y, GxObjects, bIsControlOn, bIsShiftOn);
}
